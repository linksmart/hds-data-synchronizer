...
func NewGrpcClient(serverEndpoint string, serverCertFile string) (*GrpcClient, error) {
	var dialOption grpc.DialOption
	if serverCertFile != "" {
		// Create the client TLS credentials
		creds, err := credentials.NewClientTLSFromFile(serverCertFile, "localhost:8085")
		if err != nil {
	...
	conn, err := grpc.Dial(serverEndpoint, dialOption)
	if err != nil {
		return nil, err
	}
...
func (c *GrpcClient) Query(seriesNames []string, q data.Query) (senml.Pack, error) {
...
func (c *GrpcClient) Count(series []string, q data.Query) (total int, err error) {
	request := _go.QueryRequest{
		Series:          series,
		From:            q.From.Format(time.RFC3339),
		To:              q.To.Format(time.RFC3339),
